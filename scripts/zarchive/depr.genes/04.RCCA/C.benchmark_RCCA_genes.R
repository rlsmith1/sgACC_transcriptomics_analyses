...
# GENE-LEVEL: Validate RCCA results using external data
...
# Identify RCCA significant genes -------------------------------
...
my_degs_hypergeometric <- f_hypergeometric(gene_list1 = sig_rcca_genes, 
                                           gene_list2 = my_degs,
...
# Run GSEA
set.seed(20240801)
my_degs_gsea <- GSEA(ranked_rcca_genes, TERM2GENE = m_gsea_my_degs, pvalueCutoff = 1) %>% as_tibble() %>% clean_names
my_degs_gsea
p_enrich <- my_degs_gsea %>% pull(pvalue)
nes_enrich <- my_degs_gsea %>% pull(nes)

# Plot position of consensus DEGs in RCCA gene z-score distribution
...
f_plot_gene_position(df = df_x_res, 
                     ...
                     plot_title = "Current analysis DEG position in RCCA results",
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "RCCA_current_DEGs_dist", .x), width = 4, height = 3)
)
...
# Run hypergeometric test for gene symbols
akula_degs_hypergeometric <- f_hypergeometric(gene_list1 = sig_rcca_genes, 
...
# Run GSEA
set.seed(20240801)
akula_degs_gsea <- GSEA(ranked_rcca_genes, TERM2GENE = m_gsea_akula_degs, pvalueCutoff = 1) %>% as_tibble() %>% clean_names
akula_degs_gsea
p_enrich <- akula_degs_gsea %>% pull(pvalue)
nes_enrich <- akula_degs_gsea %>% pull(nes)

# Plot position of Akula DEGs in RCCA gene z-score distribution
...
f_plot_gene_position(df = df_x_res, 
                     ...
                     x_axis_variable = "structure correlation",
                     plot_title = "Akula DEG position in RCCA results",
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "RCCA_Akula_DEG_dist", .x), width = 4, height = 3)
)
...
# Run hypergeometric test for gene symbols
consensus_degs_hypergeometric <- f_hypergeometric(gene_list1 = sig_rcca_genes, 
...
# Run GSEA
set.seed(20240801)
consensus_degs_gsea <- GSEA(ranked_rcca_genes, TERM2GENE = m_gsea_consensus_degs, pvalueCutoff = 1) %>% as_tibble() %>% clean_names
consensus_degs_gsea
...
f_plot_gene_position(df = df_x_res, 
                     ...
                     x_axis_variable = "structure correlation",
                     plot_title = "Consensus DEG position in RCCA results",
                     max_text_overlaps = 20) +
    labs(caption = paste0(
        "phyper = ", round(phyper, 2), "; n sig overlap = ", n_overlap,
        "\np GSEA = ", round(p_enrich, 2), "; NES = ", round(nes_enrich, 2)
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "RCCA_consensus_DEG_dist", .x), width = 4, height = 3)
)
...
# Run hypergeometric test for gene symbols
common_variants_hypergeometric <- f_hypergeometric(gene_list1 = sig_rcca_genes, 
...
# Run GSEA
set.seed(20240801)
common_variants_gsea <- GSEA(ranked_rcca_genes, TERM2GENE = m_gsea_common_variants, pvalueCutoff = 1) %>% as_tibble() %>% clean_names
...
f_plot_gene_position(df = df_x_res, 
                     ...
                     x_axis_variable = "structure correlation",
                     plot_title = "Common variant position in RCCA results",
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "RCCA_common_variant_dist", .x), width = 4, height = 3)
)
...
# Run hypergeometric test for gene symbols
rare_variants_hypergeometric <- f_hypergeometric(gene_list1 = sig_rcca_genes, 
...
# Run GSEA
set.seed(20240801)
rare_variants_gsea <- GSEA(ranked_rcca_genes, TERM2GENE = m_gsea_rare_variants, pvalueCutoff = 1) %>% as_tibble() %>% clean_names
rare_variants_gsea
...
f_plot_gene_position(df = df_x_res, 
                     ...
                     x_axis_variable = "structure correlation",
                     plot_title = "Rare variant position in RCCA results",
                     max_text_overlaps = 27) +
    labs(caption = paste0(
        "phyper = ", round(phyper, 2), "; n sig overlap = ", n_overlap,
        "\np GSEA = ", round(p_enrich, 2), "; NES = ", round(nes_enrich, 2)
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "RCCA_rare_variant_dist", .x), width = 4, height = 3)
)
