...
top_neg_gene <- df_x_res %>% dplyr::filter(pearsons_r < 0) %>% arrange(-abs(pearsons_r)) %>% head(1) %>% pull(ensembl_gene_id) # CX3CR1
df_vsd_regress %>% 
    ...
    mutate(gene_symbol = factor(gene_symbol, levels = c("EZH1", "CX3CR1"))) %>% 
...
df_vsd_regress %>% 
    ...
    dplyr::filter(gene_symbol == "CX3CR1") %>% 
...
f_plot_module_hypergeometric(df_grcca_module_hypergeometric,
                             y_axis = "p-adj",
                             plot_title = "Module overrepresentation of GRCCA genes",
                             module_text_size = 10,
                             subset_x_labs = TRUE,
...
sig_modules <- df_grcca_module_hypergeometric %>% dplyr::filter(p_adj < 0.01) %>% pull(module) %>% sort()
...
sig_modules <- df_grcca_module_hypergeometric %>% dplyr::filter(p_adj < 0.01) %>% arrange(module) %>% pull(module) %>% as.character()
df_go_res <- map_dfr(
    .x = sig_modules,
...
# Gene set enrichment analyses (full GRCCA gene list) ---------------------------------------------------
...
f_plot_gsea_by_ontology(df_gsea_res, 
                        ...
                        pathway_text_overlaps = 50,
                        plot_title = "GRCCA GSEA enrichments",
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "GRCCA_GSEA_enrichment", .x), width = 6, height = 10)
)
