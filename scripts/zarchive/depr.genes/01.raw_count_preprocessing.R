...
df_raw_counts_filtered <- df_raw_counts %>% 
  filter(ensembl_gene_id %in% keep)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/for_manuscript/qSVA/PCA1_GENES_normalized_counts", .x),
                width = 10, height = 5)
)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/for_manuscript/qSVA/PCA1_GENES_covariate_correlations", .x),
                width = 5, height = 8)
)



# STEP 1: regress qSVs qSV1-7 + 9 from gene level --------------------------------------------


# START WITH REGRESSING ONLY qSVS THAT MEETING CRITERIA, ALSO SEX & RACE BECAUSE WE DON'T WANT MODULES RELATED TO THESE VARIABLES
...
p_pca / p_var + plot_annotation(title = "PCA 2: VST normalized counts with qSV + sex at birth + race regression")
...
# STEP 2: regress technical covariates that still significantly correlate with significant PCs --------------------------


# REGRESS qSVs THAT MEET CRITERIA AND AGE_DEATH
df_qsvs %>% dplyr::select(-sample) %>% colnames
keep_qsvs <- paste0("qSV", c(seq(1, 7), 9)) # correlate with known covariates or var explained > 2%
mod <- df_covar_qsv %>% 
...
# PCA3: vsd ~ qSV1-7 + 9 + sex_at_birth + race + age_death --------------------------------------


# PCA ON NORMALIZED & qSV + age_death REGRESSED COUNTS    
pca3 <- prcomp(df_vsd_regress %>%
                 column_to_rownames("sample") %>% 
                 t(),
               center = TRUE, scale = TRUE)

df_pca <- pca3$rotation %>% 
  as.data.frame %>% 
  rownames_to_column("sample") %>% 
  as_tibble() %>% 
  left_join(df_covar_qsv) %>% 
...
df_pc_covar_cor %>% 
  ...
  scale_color_manual(values = c("yes" = "black", "no" = "transparent"), guide = "none") +
  guides(fill = guide_colorbar(title = "Pearson's r")) +
  labs(x = "PC (> 2% explained var)", title = "PC-covariate correlations following qSV + sex at birth + race + GC percent + age at death regression",
       caption = "Box outline indicates FDR < 0.05") +
  theme_classic() +
  theme(legend.key.height = unit(2, "cm"))
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/for_manuscript/qSVA/PCA3_GENES_covariate_correlations", .x),
                width = 8, height = 8)
)
