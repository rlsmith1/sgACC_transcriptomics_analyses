...
# CALL THE NETWORK TOPOLOGY ANALYSIS FUNCTION
  ...
  # save
  save(df_sft, file = paste0(base_dir, "objects/", prefix, "_SFT.RDS"))

# PLOT RESULTS   

  #load(paste0(base_dir, "objects/", prefix, "_SFT.RDS"))
  # scale-free topology fit index as a function of the soft-thresholding power
  p1 <- df_sft %>% 
    ggplot(aes(x = power, y = -sign(slope)*sft_r_sq)) +
    geom_text(aes(label = power), size = 4) +
  ...
  p1 + p2 + plot_annotation(title = "Soft-thresholding power fit to data")
  
  # SAVE
  map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(base_dir, "outputs/figures/for_manuscript/genes/SFT", .x),
                  width = 8, height = 4)
  )
...
# PLOT MODULE SIZES
  ...
  map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(base_dir, "outputs/figures/for_manuscript/genes/", prefix, "_sft", soft_power, "_module_sizes", .x), width = 12, height = 5)
  )
...
# write to csv  
  df_modules %>% 
    ...
    write.csv(paste0(base_dir, "outputs/tables/for_manuscript/", prefix, "_SIGNED_SFT", soft_power, "_MODS.csv"),
              row.names = FALSE)
