...
# OVERALL transcript level GRCCA results from Agoston's toolkit
...
prefix <- "15Nov2023_OVERALL_regress_qSV1.7_ageDeath_RNAbatch_GCperc"

soft_power <- 2
minimum_size <- 40
tree_cut_height <- 0.984
...
# Add gene level information to transcript modules
...
type <- "OVERALL/"
project_dir <- paste0("sft", soft_power, "_minSize", minimum_size, "_cutHeight", tree_cut_height, "_", 
                      n_mca_dim, "MCA_suicide_regressAgeDeathBrainWeight_NOGRAY/")
...
# run GRCCA on cluster ----------------------------------------------------

# IN LOCAL SPACE to sync to cluster:
# cd /Users/smithral/Documents/PhD/projects/sgacc_wgcna_grcca/RCCA_toolkit/OVERALL/
# rsync -r sft2_minSize40_cutHeight0.984_8MCA_suicide_regressAgeDeathBrainWeight_NOGRAY smithral@helix.nih.gov:/data/smithral/sgacc_wgcna/RCCA_toolkit/OVERALL
...
# cd /Users/smithral/Documents/PhD/projects/sgacc_wgcna_grcca/RCCA_toolkit/OVERALL/sft2_minSize40_cutHeight0.984_8MCA_suicide_regressAgeDeathBrainWeight_NOGRAY
# rsync -r smithral@helix.nih.gov:/data/smithral/sgacc_wgcna/RCCA_toolkit/OVERALL/sft2_minSize40_cutHeight0.984_8MCA_suicide_regressAgeDeathBrainWeight_NOGRAY/framework .
...
analysis_dir <- "grcca_permutation_VARx0.1_1_L2x0.9999_groupL2x0.1"
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/OVERALL/genes_yres", .x),
                width = 3.5, height = 3)
)
...
# top weighted transcripts ------------------------------------------------------

df_x_res %>% arrange(-abs(z_score))

df_x_res %>% 
  filter(abs(z_score) > 4.5) %>% 
  #arrange(-abs(weight)) %>% 
