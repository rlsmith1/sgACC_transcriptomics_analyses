...
n_transcripts <- df_modules_filt %>% pull(ensembl_transcript_id) %>% unique %>% length
n_genes <- df_modules_filt_geneLevel %>% pull(ensembl_gene_id) %>% unique %>% length
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/OVERALL/module_sizes", .x), width = 10, height = 4)
)
...
df_mods_go %>% 
  ...
  ggplot(aes(x = -log10(p_adj), 
             y = reorder_within(str_wrap(term, width = 25), -log10(p_adj), module))) +
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/OVERALL/sft", soft_power, "_GO_RES", .x),
                width = 18, height = 10)
)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/OVERALL/topic_modeling", .x),
                width = 10, height = 5)
)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/OVERALL/cell_type", .x),
                width = 6, height = 5)
)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/OVERALL/developmental_expression_trajectory", .x),
                width = 12, height = 6)
)
