...
prefix <- "15Nov2023_COMMON_regress_qSV1.7_ageDeath_RNAbatch_GCperc"

soft_power <- 2
minimum_size <- 40
tree_cut_height <- 0.97
...
# 26,237 transcripts map to 18,389 genes
...
df_modules_filt %>% 
  ...
  labs(title = "Number of transcripts & genes in each module",
       caption = "25,000 transcripts map to 13,530 unique genes") +
  theme(legend.position = "none")

map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/COMMON/module_sizes", .x), width = 8, height = 4)
)
...
df_combos <- df_modules_filt_geneLevel %>% 
  ...
  distinct() %>% 
  mutate(mod_set = "2_40_0.97", .before = 1)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/COMMON/sft", soft_power, "_GO_RES", .x),
                width = 18, height = 10)
)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/COMMON/topic_modeling", .x),
                width = 10, height = 5)
)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/COMMON/cell_type", .x),
                width = 6, height = 5)
)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/COMMON/developmental_expression_trajectory", .x),
                width = 12, height = 6)
)
