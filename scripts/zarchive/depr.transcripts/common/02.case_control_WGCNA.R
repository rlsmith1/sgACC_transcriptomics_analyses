...
# Greater than 100 counts in 80% of samples

common <- df_transcript_raw_counts %>% 
  mutate_if(is.numeric, ~ifelse(.x > 100, 1, 0)) %>% 
...
df_vsd_regress_common <- df_vsd_regress %>% dplyr::select(sample, any_of(common))

# SAVE  
prefix2 <- "15Nov2023_COMMON_regress_qSV1.7_ageDeath_RNAbatch_GCperc"
save(df_vsd_regress_common, file = paste0(base_dir, "objects/", prefix2, ".RDS")) # df_vsd_regress_COMMON


# SAVE AS A MATRIX FOR THE CLUSTER
count_matrix <- df_vsd_regress_common %>% column_to_rownames("sample")
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/COMMON/sft", .x),
                width = 8, height = 4)
)


# DEPR calculate adjacency matrix, TOM, clustering ------------------------------------


# LOAD DATA
base_dir <- "/Users/work/Library/CloudStorage/OneDrive-NationalInstitutesofHealth/projects/WGCNA/acsg_wgcna/"
prefix2 <- "20230315_185samples_25kCOMMONtranscripts_vst_qSVA123567_MP_RNAbatch_Race_resids"
load(paste0(base_dir, "objects/", prefix2, ".RDS")) # df_vsd_regress_COMMON
...
# m_vsd_regress_t <- df_vsd_regress_COMMON %>%
#   pivot_wider(id_cols = sample,
#               names_from = ensembl_transcript_id,
#               values_from = resids) %>%
#   as.data.frame %>% column_to_rownames("sample")
# 
# rownames(m_vsd_regress_t) <- df_vsd_regress_COMMON %>% pull(sample) %>% unique
# colnames(m_vsd_regress_t) <- df_vsd_regress_COMMON %>% pull(ensembl_transcript_id) %>% unique
...
df_modules <- map2_dfr(.x = df_combos$min_size,
                       ...
                       .f = ~ tibble(sft = soft_power,
                                     min_size = .x,
                                     cut_height = .y,
                                     ensembl_transcript_id = unique(df_vsd_regress_COMMON$ensembl_transcript_id),
...
df_module_sizes %>% filter(cut_height < 0.976) %>% 
  ...
  ggtitle(paste0("COMMON SFT", soft_power, " module sizes"))

df_module_sizes %>% filter(module == 0) %>% dplyr::count(min_size, cut_height)


map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/transcripts/COMMON/sft", soft_power, "_module_sizes", .x), width = 12, height = 5)
)
