...
# Identify rare vs common transcripts -------------------------------------
...
rare_transcripts <- df_transcript_raw_counts %>% 
    dplyr::filter(ensembl_transcript_id %in% ensembl_transcript_ids) %>% 
...
# Are implicated transcripts rare or common? ------------------------------


# Significant transcripts identified as significant in GRCCA analysis
grcca_transcripts <- df_x_res %>%
  dplyr::filter(p_adj < 0.05 & abs(z_score) >= 2) %>%
  pull(ensembl_transcript_id) 
length(grcca_transcripts) # 199


## Hypergeometric test
f_hypergeometric(
    gene_list1 = grcca_transcripts,
    gene_list2 = rare_transcripts,
    gene_universe = ensembl_transcript_ids
)

## GSEA enrichment

# Get ranked transcript list
my_grcca_transcripts_ranked <- df_x_res %>% 
    arrange(-pearsons_r) %>% 
    dplyr::select(ensembl_transcript_id, pearsons_r) %>% 
    deframe
rare_vs_common <- df_transcript_to_gene %>% 
    mutate(prevalence = ifelse(ensembl_transcript_id %in% rare_transcripts, "rare", "common")) %>% 
    dplyr::select(prevalence, ensembl_transcript_id) %>% 
    group_by(prevalence) %>% 
    summarise(named_list = list(ensembl_transcript_id)) %>% 
    deframe
    
fgsea(pathways = rare_vs_common, 
      stats = my_grcca_ranked, 
      eps = 0
)  %>% 
    as_tibble() %>% 
    dplyr::rename("prevalence" = "pathway") %>% 
    arrange(-abs(NES))
...
sig_gene_ids <- df_x_res %>% 
    dplyr::filter(significant == 1 & !is.na(ensembl_gene_id)) %>% 
    pull(ensembl_gene_id)
sig_gene_ids_multiple <- df_x_res %>% 
    dplyr::filter(ensembl_gene_id %in% sig_gene_ids) %>% 
    dplyr::count(ensembl_gene_id, sort = TRUE) %>% 
    dplyr::filter(n > 1) %>% 
    pull(ensembl_gene_id)

df_x_res %>% 
    #dplyr::filter(ensembl_gene_id == "ENSG00000099622") %>%
    dplyr::filter(ensembl_gene_id %in% sig_gene_ids_multiple) %>% 
    mutate(label = ifelse(significant == 1, "*", "")) %>% 
    
    ggplot(aes(x = pearsons_r, y = reorder(transcript_symbol, pearsons_r))) +
    geom_vline(xintercept = 0, color = "gray") +
    geom_point(aes(color = pearsons_r, size = -log10(p_adj))) +
    geom_text(aes(label = label), color = "white", vjust = 0.75, size = 6) +
    facet_wrap(vars(gene_symbol), scale = "free_y") +
