...
# GENE-LEVEL: Run GRCCA with 1 diagnosis column indicating if the individual was
# diagnosed with -any- of SCZ, BD, or MDD (1) or no psychiatric diagnosis (0)
...
# Cross-disorder GRCCA model significance -------------------------------
...
df_xdx_res <- map(
    ...
    bind_rows(df_results %>% mutate(Analysis = "Actual")) %>% # Original analysis (SCZ, BD, MDD)
    bind_rows(l_grcca_res_comp$model_results %>% mutate(Analysis = "Composite")) %>% # Composite case
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "R3C3A.cross_disorder_model_results", .x), width = 6.5, height = 2.5)
)



# Cross-disorder covariate (Y) results ------------------------------------
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "R3C3B.cross_disorder_covariate_results", .x), width = 2.5, height = 3.7)
)



# Gene-level comparison (correlation) ---------------------------------------------------
...
df_xdx_xres <- map(
    ...
    list_rbind(names_to = "Analysis") %>%
    bind_rows(l_grcca_res_comp$x_res %>% mutate(Analysis = "Composite")) %>% # Composite case
    ...
    bind_rows(df_x_res %>% mutate(Analysis = "Actual")) %>% # Original results (SCZ, BD, MDD)
    mutate(
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "R3C3C.cross_disorder_gene_corr", .x), width = 4.0, height = 3.5)
)



# Gene-level overlaps (after threshold for significance) ---------------------------------------------------
...
df_xres_upset <- df_xdx_xres %>%
    ...
    arrange(Analysis) %>% 
    pivot_wider(id_cols = ensembl_gene_id, names_from = Analysis, values_from = significant) %>% 
...
#     filter(significant == 1) %>% 
#     group_split(Analysis) %>%
#     map( ~ pull(.x, gene_symbol)) %>%
#     set_names(unique(df_xdx_xres$Analysis))
# 
# 
# ## Venn diagram
# ggvenn(
#     data = l_sig_genes,
#     # Specify the names of the list elements to plot
#     columns = c("Actual", "BD", "MDD", "Composite"),
#     
#     # Customization for aesthetics
#     fill_color = pal_nejm()(4), # Set distinct colors
#     stroke_size = 0.5,
#     set_name_size = 4, # Size of the set labels (BD, MDD, etc.)
#     text_size = 3.5 # Size of the intersection counts
# ) +
#     labs(
#         title = "C | Overlap of Significant Gene Sets"
#     ) +
#     theme(
...
# map(
#     .x = c(".png", ".pdf"),
#     .f = ~ ggsave(paste0(figures_dir, "R3C3C.cross_disorder_gene_vennDiagram", .x), width = 2.5, height = 4)
# )
