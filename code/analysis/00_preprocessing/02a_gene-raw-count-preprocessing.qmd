---
title: "00-02a. Genes raw count preprocessing"
author: "Rachel Smith"
format: html
editor: visual
---
...
# map(
#   .x = c(".png", ".pdf"),
#   .f = ~ ggsave(paste0(supp_figures_dir, "PCA1_GENES_normalized_counts", .x),
#                 width = 10, height = 5)
# )
```
...
# map(
#   .x = c(".png", ".pdf"),
#   .f = ~ ggsave(paste0(supp_figures_dir, "PCA2_GENES_normalized_and_regressed_counts", .x),
#                 width = 6, height = 8)
# )
...
df_pc_covar_cor %>% 
  ...
  labs(x = "PC (> 2% explained var)", 
       title = "PC-covariate correlations following qSV regression",
...
# map(
#   .x = c(".png", ".pdf"),
#   .f = ~ ggsave(paste0(base_dir, "supp_figures_dir/PCA2_GENES_covariate_correlations", .x),
#                 width = 8, height = 8)
# )
```
...
```{r regress_step3}
df_qsvs %>% dplyr::select(-sample) %>% colnames
# correlate with known covariates or var explained > 2%
...
p_pca / p_var + 
    plot_annotation(title = "PCA 3: VST normalized counts with qSV + sex at birth + race + GC percent + age at death regression")


## Save
# map(
#   .x = c(".png", ".pdf"),
#   .f = ~ ggsave(paste0(base_dir, "supp_figures_dir/PCA3_GENES_normalized_and_regressed_counts", .x),
#                 width = 6, height = 8)
# )
```
...
### Figure S2C \| PC-covariate correlations before and after covariate regressions
...
df_figS2c %>% 
    ...
    facet_grid2(regression ~ type, scales = "free", space = "free_y") +
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supp_figures_dir, "S2C.PCA_covariate_cor", .x), 
                width = 6.5, height = 4)
)
```
