...
## This script is similar to 02a, but because the final model includes the 
## same covariates as the gene-level analysis (for the sake of consistency),
## we just compare normalized count data to normalized + corrected count data
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/PCA1_TRANSCRIPTS_covariate_correlations", .x),
                width = 5, height = 8)
)



# STEP 2: REGRESS covriates from transcript level data --------------------------------------------
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(base_dir, "outputs/figures/PCA3_TRANSCRIPTS_normalized_and_regressed_counts", .x),
                  width = 7, height = 8)
)
