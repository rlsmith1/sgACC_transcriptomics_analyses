---
title: "04b. Rerun toxicology MCA treating missing variables by filling in the individual-level mode"
author: "Rachel Smith"
format: html
editor: visual
---

# Toxicology sensitivity analysis
...
save(df_eig, df_var_loadings, df_ind_loadings, df_mca_covar_cor,
     file = paste0(objects_dir, "R2-drug_MCA_results_modeImpute.Rdata")
)
```
...
### Figure S4A \| Matrix of drug use across 185 samples

```{r FigS4a.prep, fig.width=6.5, fig.height=2.25}
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S4A.sample_toxicology_matrix", .x), 
                  width = 6.5, height = 2.25)
)
```

### Figure S4B \| Variance explained of each toxicology MCA dimension

```{r FigS4b.plot, fig.width=2.0, fig.height=2.5}
## MCA eigenvalues
df_eig %>% 
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(supp_figures_dir, "S4B.MCA_eigenvalues", .x), 
                  width = 2.0, height = 2.5)
)
```

### Figure S4C \| MCA variable loadings

```{r FigS4c.prep}
## MCA variable loadings

## Prep data
figS4C <- df_var_loadings %>% 
...
scale_max <- figS4C %>% pull(score) %>% abs %>% max
gene_weight_color_scale <- rev(brewer.rdbu(100))
```

```{r FigS4c.plot, fig.width=4.5, fig.height=3.3}
## Plot 
figS4C %>%
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(supp_figures_dir, "R3C1C.MCA_variable_loadings", .x), 
                  width = 4.5, height = 3.3)
)
```

### Figure S4D \| MCA dimension - covariate correlations

```{r FigS4d.prep}
## MCA covariate-dimension correlations

## Prep
technical_covariates <- c("mapped_percent", "gc_percent", "five_prime_three_prime_bias", "rin_acsg", "rna_extraction_batch", "library_batch", "pmi", "pmi_confidence", "source", "ph")
biological_covariates <- c("age_death", "sex_at_birth", "race", "bmi", "height", "weight", "marital_status", "manner_death", "education", "suicide", "brain_weight")


df_figS4D <- df_mca_covar_cor %>% 
...
```{r FigS4d.plot, fig.width=6.5, fig.height=3.4}
df_figS4D %>% 
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(supp_figures_dir, "S4D.MCA_dimension_covariate_cor", .x), 
...
This ended up in the review only, not the main manuscript

```{r FigSXX, fig.width=6.5, fig.height=3.4}
## Plot
df_figS4D %>% 
...
# map(
#     .x = c(".png", ".pdf"),
#     .f = ~ ggsave(paste0(supp_figures_dir, "R3C1.MCA_dimension_covariate_cor_ALL", .x), 
#                           width = 6.5, height = 3.4)
# )
```
