...
## Identify covariates to regress from gene expression data
## and include as covariates in DGE model
## This is a stepwise script: 
## --> regress --> run PCA --> check expression PC-covariate correlations --> repeat
...
## Start by regression only qSVs that meeting significance criteria
## >2% variance explained or correlate with known covariates
## Also regress sex & race up front becuase we don't want these demographic variables influencing modules
...
## Regress same covariates as before, but this time also regress age at death and GC percent
## Because these variables still correlated with expression PCs
df_qsvs %>% dplyr::select(-sample) %>% colnames
keep_qsvs <- paste0("qSV", c(seq(1, 7), 9)) # correlate with known covariates or var explained > 2%
mod <- df_covar_qsv %>% 
