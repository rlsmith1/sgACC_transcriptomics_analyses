...
figures_dir <- paste0(base_dir, "outputs/figures/Fig1_analysisPipeline/")
...
```{r Fig1.GSEA.barcode, fig.width = 2.5, fig.height = 2}
## Load GRCCA data
...
## Pathway genes
pathway_genes <- cell_types[["Neuro-Ex"]]

## Calculate the running enrichment score
n <- length(my_grcca_ranked)
m <- length(pathway_genes)
is_hit <- names(my_grcca_ranked) %in% pathway_genes

## Calculate the weight of hits and non-hits
hit_weights <- abs(my_grcca_ranked[is_hit])^1 # Power = 1 is standard
P_hit <- cumsum(is_hit * abs(my_grcca_ranked)^1) / sum(hit_weights)
P_no_hit <- cumsum(!is_hit) / (n - m)
running_es <- P_hit - P_no_hit

## Create tibble to plot
plot_df <- tibble(
  rank = 1:n,
  rx = my_grcca_ranked,
  es = running_es,
...
## Rescale colors
limits <- c(summary(my_grcca_ranked)[1], summary(my_grcca_ranked)[6])
zero_pos <- rescale(0, from = limits)

## Plot
ggplot(plot_df, aes(x = rank, y = es)) +
  geom_line(color = "darkorchid", size = 0.75) + 
  geom_hline(yintercept = 0, color = "black", size = 0.5) +
  geom_segment(data = filter(plot_df, is_hit == TRUE),
               aes(x = rank, xend = rank, y = -0.05, yend = 0, color = rx),
               alpha = 0.5) +
  # Formatting
  theme_classic() +
    scale_color_gradientn(colors = brewer.rdbu(100) %>% rev,
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "4.GSEA_barcode", .x), width = fig_width, height = fig_height)
)
```
