...
#----------------FOLLOWING SECOND ROUND REVISIONS------------------------------#
...
```{r}
## Set new figure output directory for now
figures_dir <- paste0(base_dir, "outputs/figures/response_to_reviewers/round2/")

## Make sure original MCA data are loaded
load(paste0(analysis_objects_dir, "drug_MCA_results.Rdata")) # df_eig, df_var_loadings, df_mca_covar_cor
```
...
dx_sample_size <- df_figS3a %>% dplyr::select(sample, dx) %>% distinct %>% count(dx) %>% deframe
```
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S3A.sample_toxicology_matrix", .x), width = 6.5, height = 2.25)
    #.f = ~ ggsave(paste0(supplement_figures_dir, "S3A.sample_toxicology_matrix", .x), width = 6.5, height = 2.25)
)
```
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S3B.MCA_variance_explained", .x), width = 2.0, height = 2.5)
    #.f = ~ ggsave(paste0(supplement_figures_dir, "S3B.MCA_variance_explained", .x), width = 2.0, height = 2.5)
)
```
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S3C.MCA_variable_loadings", .x), width = 4.5, height = 3.35)
...
df_figS3d <- df_mca_covar_cor %>% 
    ...
    filter(covariate %in% c(names(dx_colors), biological_covariates, drug_covariates, technical_covariates)) %>% 
...
df_figS3d %>% 
    #filter(covariate != "Tecnical") %>% # Remove technical for now?
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S3D.MCA_dimension_covariate_correlations", .x), width = 6.5, height = 3.4)
