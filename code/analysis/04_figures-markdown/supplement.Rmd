...
# FIG S10: Transcript model results (subset and full) #
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S10A.optimal_model_all_transcripts", .x), width = fig_width, height = fig_height)
)
```
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S10B.covariate_correlations_all_transcripts", .x), width = fig_width, height = fig_height)
)
```
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S10C.optimal_model_subset_transcripts", .x), width = fig_width, height = fig_height)
)
```



# FIG S11: GRCCA results are related to single-cell expression gradients implicated in SCZ #

```{r FigS11.prep}
df_figS11 <- df_lf4_grcca %>% 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S11.correlation_with_SNAP", .x), width = 6, height = fig_height)
)
```


# FIG S12: WGCNA module - phenotype associations determined through traditional module eigengene analysis #

### A | Module eigengene - covariate significant associations

```{r FigS12a.prep}
df_figS12a <- df_lm_dx_allPCs %>% 
...
```{r FigS12a.plot, fig.height = 3, fig.width = 6.5}
df_figS12a %>% 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S12a.module_kme_covar_matrix", .x), width = 6.5, height = 3)
)
```
...
```{r FigS12b.plot, fig.height = 3, fig.width = 6.5}
df_figS12b %>% 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S12B.sig_SCZ_kME_modules", .x), width = 6.5, height = 3)
)
```

# FIG S13: WGCNA module enrichment for risk gene lists #

```{r FigS13.prep}
df_figS13 <- df_mods_risk_gene_hypergeometric %>% 
...
```{r FigS13.plot, fig.width = 5, fig.height = 5}
df_figS13 %>% 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S13.risk_gene_module_hypergeometric", .x), width = 5, height = 5)
)
```


# FIG S14: Functional enrichment of GRCCA overrepresented WGCNA modules #
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S14A.GRCCA_module_hypergeometric", .x), width = 2.15, height = 5)
)
```
...
```{r FigS14b.plot, fig.height = 5, fig.width = 2.15}
df_x_res %>% 
    dplyr::filter(module %in% sig_modules & ensembl_gene_id %in% sig_grcca_genes) %>% 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S14B.sig_module_direction", .x), width = 2.15, height = 5)
)
...
```{r FigS14c.bubbleplot, fig.height = 5, fig.width = 2.15}
df_figS14c %>% 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S14C.sig_GRCCA_module_enrichments", .x), width = 2.15, height = 5)
)
```


# FIG S15: Compare GRCCA and RCCA directly #
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S15A.covariate_structCor_correlation", .x), width = fig_width, height = fig_height)
)
```
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S15B.gene_structCor_correlation", .x), width = fig_width, height = fig_height)
)
```
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S15D.Risk_gene_enrichment", .x), width = fig_width, height = fig_height)
)
```


# FIG S16: Compare across WGCNA module solutions

### A | Model significance
```{r FigS16a, fig.width = 6.5, fig.height = 2}
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S16A.wgcna_groups_model_significance", .x), width = 6.5, height = 2)
)
```

### B | X and Y results correlations
```{r FigS16b, fig.width = 4.0, fig.height = 3.75}
# Plot
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S16B.wgcna_groups_x_y_cor", .x), width = 4.0, height = 3.75)
)
```

### C | Enrichment for risk genes
```{r FigS16c, fig.height = 3.75, fig.width = 2.4}
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S16C.wgcna_groups_benchmarking", .x), width = 2.4, height = 3.75)
)
```


# FIG S17: Compare across random module solutions

### A | Model significance
```{r FigS17a, fig.width = 6.5, fig.height = 2}
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S17A.random_groups_model_significance", .x), width = 6.5, height = 2)
)
```

### B | X and Y results correlations
```{r FigS17b, fig.width = 6.5, fig.height = 2}
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S17B.random_groups_x_y_cor", .x), width = 6.5, height = 2)
)
```


### C | Enrichment for risk genes
```{r FigS17c, fig.height = 2, fig.width = 6.5}
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S17C.random_groups_benchmarking", .x), width = 6.5, height = 2)
)
```


# FIG S18: Developmental expression trajectories of GRCCA genes
...
df_figS18 <- df_psychencode_grcca %>% #dplyr::filter(regioncode == "MFC") %>% 
...
```{r FigS18, fig.height = 3, fig.width = 4}
df_figS18 %>% 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supplement_figures_dir, "S18.developmental_trajectory", .x), width = 4, height = 3)
)
```
