---
title: "02c-01. Define X.mat, Y.mat, and C.mat to include in transcript-level GRCCA analysis"
author: "Rachel Smith"
format: html
editor: visual
---

Select samples and variables for (G)RCCA. Define X (transcript expression), Y (covariates/confounds), and C (confounds if separated).

This is pretty much the same as the gene-level GRCCA script.
...
```{r load_data}
source(paste0(scripts_dir, "load_WTCNA_res.R"))
```
...
YC.mat <- df_covariates_numeric %>%
    ...
    # create dx columns
    mutate(
        Control = ifelse(grepl("control", sample), 1, 0),
...
if(!dir.exists(cca_dir)) dir.create(cca_dir, recursive = TRUE)
cca_data_dir <- paste0(cca_dir, "data/")
if(!dir.exists(cca_data_dir)) dir.create(cca_data_dir)
