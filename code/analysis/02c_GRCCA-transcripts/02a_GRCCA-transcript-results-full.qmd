---
title: "02a. Load transcript-level GRCCA results (FULL)"
author: "Rachel Smith"
format: html
editor: visual
---
...
### Figure S13A \| Optimal model for all transcripts
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supp_figures_dir, "S13A.optimal_model_all_transcripts", .x), width = 6.5, height = 5)
)
```

### Figure S13B \| Covariate correlations with the latent variable (all transcripts)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supp_figures_dir, "S13B.covariate_correlations_all_transcripts", .x), 
                width = 6.5, height = 5)
)
```
