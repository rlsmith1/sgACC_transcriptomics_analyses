---
title: "02c-02b. Load transcript-level GRCCA results (SUBSET)"
author: "Rachel Smith"
format: html
editor: visual
---
...
```{r save}
# Figures
save(df_results_transcripts, df_lvs_transcripts, 
     df_y_res_transcripts, df_x_res_transcripts,
    file = paste0(objects_dir, "GRCCA_transcript_results.Rdata")
)
```

## Export transcript-level GRCCA results as supplement table
...
write_xlsx(list("A.GRCCA results" = df_results_transcripts %>% 
                ...
                "C. Transcript (X) results" = df_x_res_transcripts %>% 
                    dplyr::rename("structure_correlation" = "pearsons_r")), 
...
### Figure S5A \| Covariate structure correlations (ry)
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "5A.covariate_structCor", .x), 
                width = 2.5, height = 2)
)
```

### Figure S5B \| Correlation with gene-level GRCCA results
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "5B.gene_vs_transcript_GRCCA_res", .x), 
                width = 2.5, height = 2)
)
```

### Figure S5C \| Directionality of transcripts associated with significant and non-significant risk genes
...
df_fig5c %>%
    ...
    scale_fill_gradientn(colors = gene_weight_color_scale, 
                         limits = c(-0.278, 0.278), guide = "none") +
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "5C.directionality_of_transcripts", .x), 
                width = 3.75, height = 4.15)
)
```

### Figure S17C \| Optimal model for subset transcripts

```{r FigS17c.plot, fig.width=6.5, fig.height=5}
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supp_figures_dir, "S17C.optimal_model_subset_transcripts", .x),
                width = 6.5, height = 5)
)
```
