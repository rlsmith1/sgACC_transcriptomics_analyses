---
title: "02c-00. Run WGCNA pipeline on transcripts"
author: "Rachel Smith"
format: html
editor: visual
---
...
**NOTE: Run this script on an HPC cluster!!!**
...
load(paste0(objects_dir, prefix2, ".RDS")) # df_vsd_regress_rare
load(paste0(pbjects_dir, prefix2, "_matrix.RDS")) # m_vsd_regress_t

### Make sure to run on HPC ###
...
# SAVE
save(df_modules, file = paste0(objects_dir, prefix2, "_SIGNED_SFT", soft_power, "_MODS.RDS"))
```
...
# PLOT MODULE SIZES
soft_power <- 2
load(paste0(objects_dir, prefix2, "_SIGNED_SFT", soft_power, "_MODS.RDS")) # df_modules
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supp_figures_dir, "transcripts_sft", soft_power, "_module_sizes", .x), 
                width = 12, height = 6)
)
```
...
df_modules %>%
  ...
  arrange(module) %>%
  write.csv(
      paste0(base_dir, "outputs/tables/TableSXX", prefix2, "_SIGNED_SFT", soft_power, "_MODS.csv"),
    row.names = FALSE
  )
```
