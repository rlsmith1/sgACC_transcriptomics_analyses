---
title: "02b-01. Load and formatGRCCA results"
author: "Rachel Smith"
format: html
editor: visual
---
...
Run permutation analysis to test the significance of rx association in each diagnostic group.
...
## Export results table for supplement
...
df_results %>% 
    ...
    guides(size = guide_legend(title = "X-Y LV correlation", 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "2A.optimal_var_explained", .x), 
                width = 2.5, height = 2)
)
```
...
df_y_res %>% 
    ...
    geom_text(aes(label = significant), 
              size = 6, vjust = 0.75, hjust = -1.5, color = "black") +
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "2B.covariate_structCor", .x), 
                width = 4.0, height = 2)
)
```
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "2C.gene_structCor", .x), 
                width = 2.5, height = 4)
)
```
...
```{r Fig2d.stats}
load(paste0(objects_dir, "DE_results.Rdata")) # df_de_res (Using Rdata instead of RDS as per 00_DGE output)
# Note: This file isn't create until 03_DGE (it's a bit of a sneak peek :) )
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "2D.top_gene_expression_values", .x), 
                width = 4, height = 2)
)
```
...
df_fig2e %>% 
    ...
    coord_cartesian(clip = "off") +
    labs(x = "Mean expression (corrected)", 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "2E.gene_strucCor_vs_Expr", .x), 
                width = 4.0, height = 2)
)
```

### Figure S9 \| Covariate weights
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supp_figures_dir, "S9.geneLevel_covariate_weights", .x), 
                width = 2.5, height = 2)
)
```

### Figure S10 \| Vector of significant GRCCA genes (FDR_r \< 0.05 & Z-score \> 2)

```{r FigS10.plot, fig.height = 3}
df_x_res %>% 
    dplyr::filter(p_adj < 0.05 & abs(z_score) >= 2) %>% 
    ...
    xlim(c(-1, 1)) +
    labs(y = "Gene", 
         x = expression("Structure correlation (" * italic(r)[x] * ")"), 
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(supp_figures_dir, "S10.significant_GRCCA_genes_all", .x), 
                width = 2.5, height = 3)
)
```
