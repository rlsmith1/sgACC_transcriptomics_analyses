---
title: "02b-06d. Sensitivity analyses: Run GRCCA across mu values to demonstrate sensitivity of results to group-level regularization"
author: "Rachel Smith"
format: html
editor: visual
---

Demonstrate sensitivity of the main LV to the group penalty (mu ∈ {0.001 0.01 0.5 0.9 0.999}; quasi-logarithmic scale).
...
load(paste0(objects_dir, "GRCCA_results.Rdata")) # df_lvs, df_results, df_y_res, df_x_res, df_rx_expr_cor
...
### Figure S14A \| Mu sensitivity model comparison

```{r FigS14A.plot, fig.width=6.5, fig.height=2.1}
# Plot
df_results_muSENS %>% 
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(supp_figures_dir, "S14A.mu_sensitivity_model_results", .x), 
                  width = 6.5, height = 2.1)
)
```

### Figure S14B \| Mu sensitivity covariate results

```{r FigS14B.plot, fig.width=6.5, fig.height=2.1}
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(supp_figures_dir, "S14B.mu_sensitivity_covariate_results", .x), 
                  width = 6.5, height = 2.1)
)
```

### Figure S14C \| Gene structure correlations across mu values

```{r FigS14C.plot, fig.width=6.5, fig.height=1.75}
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(supp_figures_dir, "S14C.mu_sensitivity_gene_results", .x), 
                  width = 6.5, height = 1.75)
)
```

### Figure S14D \| Gene-level overlaps (after threshold for significance)

Thresholded for significance

```{r FigS14D.plot, fig.width=6.5, fig.height=3}
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(supp_figures_dir, "S14D.mu_sensitivity_gene_upset", .x), 
                  plot = p_S14d,
                  width = 6.5, height = 3)
)
```
