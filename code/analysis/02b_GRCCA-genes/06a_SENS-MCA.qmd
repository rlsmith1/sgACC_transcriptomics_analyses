---
title: "02b-06a. Sensitivity analyses: Run GRCCA using a different approach to imputing missing values"
author: "Rachel Smith"
format: html
editor: visual
---

Run GRCCA using a different approach to imputing missing values in the toxicology data (see `00_preprocessing/04b_tox-MCA-sensitivity.R`).
...
## Run GSEA analyses on imputed MCA results
...
### Figure S5A \| MCA sensitivity model comparison

```{r FigS5A.plot, width=3.25, height=2.5}
## Combine new and original GRCCA results
df_figS5A <- df_results %>% 
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(supp_figures_dir, "S5A.MCA_sensitivity_model_results", .x), 
                  width = 3.25, height = 2.5)
)
```

### Figure S5B \| MCA sensitivity covariate results

```{r FigS5B.plot, width=3.15, height=2.5}
covariate_order <- c(
    "SCZ", "BD", "MDD",
    paste0("Dim ", 1:8)
)
df_figSS5B <- df_y_res %>% 
    ...
    # negate ry for Dim 1-3 in the MCA sensitivity analysis 
    ## (to get everything in the same direction)
    mutate(
        pearsons_r = ifelse(
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S5B.MCA_sensitivity_covariate_results", .x), 
                  width = 3.15, height = 2.5)
)
```

### Figure S5C \| MCA sensitivity gene correlations

```{r FigS5C.plot, width=3.15, height=2.5}
## Combine new and original GRCCA results
df_figS5C <- df_x_res %>% 
    mutate(Analysis = "Original", .before = 1) %>% 
    bind_rows(
        l_grcca_res_mcaImpute$x_res %>% 
            ...
            left_join(df_modules_filt %>% dplyr::select(-c(mod_set, color)), 
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S5C.MCA_sensitivity_gene_results", .x), 
                  width = 3.15, height = 2.5)
)
```

### Figure S5D \| MCA sensitivity risk gene GSEA

```{r FigS5D.plot, width=3.15, height=2.25}
## Prep
df_figS5D <- df_grcca_fgsea_benchmark %>% 
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S5D.MCA_sensitivity_risk_gene_enrichment", .x), 
                  width = 3.15, height = 2.25)
)
```
