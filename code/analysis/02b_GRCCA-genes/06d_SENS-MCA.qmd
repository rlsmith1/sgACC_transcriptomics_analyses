---
title: "06d. Sensitivity analyses: Run GRCCA using a different approach to imputing missing values"
author: "Rachel Smith"
format: html
editor: visual
---
...
### Figure R3C1A \| MCA sensitivity model comparison

```{r FigR3C1A.plot, width=3.25, height=2.5}
...
### Figure R3C1B \| MCA sensitivity covariate results

```{r FigR3C1B.plot, width=3.15, height=2.5}
covariate_order <- c(
    "SCZ", "BD", "MDD",
    paste0("Dim ", 1:8)
)
...
### Figure R3C1C \| MCA sensitivity gene correlations

```{r FigR3C1C.plot, width=3.15, height=2.5}
## Combine new and original GRCCA results
df_figSXXC <- df_x_res %>% 
    mutate(Analysis = "Original", .before = 1) %>% 
    bind_rows(
...
### Figure R3C1D \| MCA sensitivity risk gene GSEA

```{r FigR3C1D.plot, width=3.15, height=2.25}
## Prep
