---
title: "05_transcript-annotation"
author: "Rachel Smith"
format: html
editor: visual
---

# Transcript-level annotations (Figure 5D)

Transcript-level annotations to increase the novelty and interpretability of these findings. Generates Figure 5D (CSMD1 example) and Figure S14 (PTK2B, ARHGAP44).
...
library(biomaRt)
library(ggarrow) # Assuming this package is used for geom_arrowsegment
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "D.CSMD1_example", .x), plot = fig5d, width = 6.5, height = 4.5)
)
```
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S14A.PTK2B", .x), plot = figS14a, width = 6.5, height = 4.5)
)
...
map(
    .x = c(".png", ".pdf"),
    .f = ~ ggsave(paste0(figures_dir, "S14B.ARHGAP44", .x), plot = figS14b, width = 6.5, height = 4.5)
)
...
```{r save_final}
save(fig5d, file = paste0(analysis_objects_dir, "Fig5d.Rdata"))
