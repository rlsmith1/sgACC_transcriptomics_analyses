---
title: "02a_GRCCA-transcript-results-full"
author: "Rachel Smith"
format: html
editor: visual
---

# Load transcript-level GRCCA results (FULL)
...
## Figure S13 | Transcript model results (all transcripts)

### A | Optimal model for all transcripts

```{r FigS13a.plot, fig.width=6.5, fig.height=5}
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "S13A.optimal_model_all_transcripts", .x), width = 6.5, height = 5)
)
```

### B | Covariate correlations with the latent variable (all transcripts)

```{r FigS13b.plot, fig.width=6.5, fig.height=5}
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "S13B.covariate_correlations_all_transcripts", .x), width = 6.5, height = 5)
)
```
