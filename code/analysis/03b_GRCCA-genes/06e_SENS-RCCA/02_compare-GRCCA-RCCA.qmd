---
title: "02_compare-GRCCA-RCCA"
author: "Rachel Smith"
format: html
editor: visual
---

# GENE-LEVEL: Compare RCCA vs GRCCA results
...
# Note: Using objects directory instead of figure-specific directory for reproducibility if possible, 
# or assuming the previous scripts populated the objects directory.
# Re-using the logic from the original script to load from specific locations if needed, 
# but preferably using standard locations.
# The original script loaded from: outputs/updated_figures/objects/Fig3_GRCCAres/ and Fig4...
# Let's try loading from standard analysis_objects_dir first, checking if keys exist.
load(paste0(analysis_objects_dir, "GRCCA_results.Rdata")) # df_lvs, df_y_res, df_x_res

# Load enrichment results if needed
# load(paste0(analysis_objects_dir, "GRCCA_enrichment_res.Rdata")) # if exists
```
...
## Figure S11 | Compare GRCCA and RCCA directly
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "S11A.covariate_structCor_correlation", .x), width = 6.5, height = 5)
)
```
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "S11B.gene_structCor_correlation", .x), width = 6.5, height = 5)
)
```

### C | Venn diagram of significant gene overlap
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(figures_dir, "S11D.Risk_gene_enrichment", .x), width = 6.5, height = 5)
)
```
