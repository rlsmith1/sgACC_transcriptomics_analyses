...
for i=1:nlevels
    for j=1:nmods
        ...
        switch sidvar
            ...
            case {'osplit' 'isplit'}
                % Concatenate data
                if ismember(cfg.machine.name, {'cca' 'rcca' 'grcca'})
        ...
        if ismember(cfg.machine.name, {'pls' 'spls'})
            ...
            % GRCCA inverse feature transformation
            if strcmp(cfg.machine.name, 'grcca')
                w = transform_data(cfg, 'grcca-inverse-transform', w, mod{i,j}, 'lambda', ...
                    param.(['L2' lower(mod{i,j})]), 'mu', param.(['groupL2' lower(mod{i,j})]));
            end
