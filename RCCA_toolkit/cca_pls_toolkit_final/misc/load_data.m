...
for i=1:numel(mod)
    ...
    proc = 0;
    if ~strcmp(mod{i}, 'C') && exist('param', 'var')
        if strcmp(cfg.machine.name, 'grcca') && ...
                (param.(['L2' lm]) ~= 0 && param.(['groupL2' lm]) ~= 0) && ...
                (param.(['L2' lm]) ~= param.(['groupL2' lm]))
            % Initialize folder/file for SVD results after GRCCA transformation
            fname_svd = getfname_dirload(cfg, 'grcca', lm, splittype, split, param);
        ...
        elseif contains(cfg.machine.name, 'cca')
    ...
    if proc
        ...
        if ismember(cfg.machine.name, {'pls' 'spls'}) ...
            ...
            % Transform features to reduce GRCCA problem to RCCA problem (see Tuzhilina et al. 2021)
            if strcmp(cfg.machine.name, 'grcca') && ...
                    param.(['L2' lm]) ~= 0 && param.(['groupL2' lm]) ~= 0
                trdata = transform_data(cfg, 'grcca-right-transform', trdata, m, ...
                    'lambda', param.(['L2' lm]), 'mu', param.(['groupL2' lm]));
                if exist('teid', 'var')
                    tedata = transform_data(cfg, 'grcca-right-transform', tedata, m, ...
                        'lambda', param.(['L2' lm]), 'mu', param.(['groupL2' lm]));
                end
            end
