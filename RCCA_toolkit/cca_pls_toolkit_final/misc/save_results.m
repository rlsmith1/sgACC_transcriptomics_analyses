...
%   add groupl2x/groupl2y to results table 
...
for i=1:numel(cfg.env.save.tableHeading)
    if contains(cfg.env.save.tableHeading{i}, {'nfeatx' 'nfeaty' 'npcax' 'npcay' ...
            'l2x' 'l2y' 'groupl2x' 'groupl2y' 'varx' 'vary'})
    ...
    switch cfg.env.save.tableHeading{i}
        ...
        case {'groupl2x' 'groupl2y'}
            output = [output {cat(1, param.(['groupL2' m]))}];
            ...
            if isfield(param, ['PCA' m])
                ...
                for j=1:res.frwork.split.nall
                    if strcmp(cfg.machine.name, 'grcca')
                        fname = getfname_dirload(cfg, 'grcca', m, 'osplit', res.frwork.split.all(j), param(j));
                    else
...
for i=1:numel(varargin)
    if ~mod(i, 2)
        switch varargin{i-1}
            ...
            case {'dist' 'l2x' 'l2y' 'groupl2x' 'groupl2y' 'pval' ...
                    'varx' 'vary' 'correl' 'covar'}
