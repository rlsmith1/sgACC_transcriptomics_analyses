...
%   refactor code a bit
%   use transform_data.m for GRCCA inverse feature transformation
...
for i=2:res.frwork.level
    ...
    if ismember(cfg.machine.name, {'pls' 'spls'})
        ...
        if strcmp(cfg.machine.name, 'grcca')
            w = transform_data(cfg, 'grcca-inverse-transform', w, m, 'lambda', ...
                    param.(['L2' lower(m)]), 'mu', param.(['groupL2' lower(m)]));
        end
