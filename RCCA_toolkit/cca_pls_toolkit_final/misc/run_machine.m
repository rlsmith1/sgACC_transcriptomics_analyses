...
switch cfg.machine.name
    ...
    case {'cca' 'rcca' 'grcca'}
        ...
        % CCA/RCCA/PLS/PCA-CCA/GRCCA solved by standard eigenvalue problem
...
if contains(cfg.machine.name, 'cca')
    ...
    if strcmp(cfg.machine.name, 'grcca') && param.L2x ~= 0 && param.groupL2x ~= 0
        S.wX = transform_data(cfg, 'grcca-left-transform', S.wX, 'X', 'lambda', param.L2x, 'mu', param.groupL2x);
    end
    if strcmp(cfg.machine.name, 'grcca') && param.L2y ~= 0 && param.groupL2y ~= 0
        S.wY = transform_data(cfg, 'grcca-left-transform', S.wY, 'Y', 'lambda', param.L2y, 'mu', param.groupL2y);
    end
end
