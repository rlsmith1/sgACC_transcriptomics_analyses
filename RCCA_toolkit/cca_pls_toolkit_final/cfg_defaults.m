...
%   add cfg fields for GRCCA analysis
...
% Feature groups
if strcmp(cfg.machine.name, 'grcca')
    cfg.data.group = 1;
else
    def.data.group = 0;
end
...
if strcmp(cfg.machine.name, 'grcca')
    def.env.save.svd = 0; % do not save SVD results due to heavy disc load
else
...
% Data path, preprocessing and groups
if cfg.data.conf
...
for i=1:numel(mod)
    ...
    if cfg.data.group
        if exist(fullfile(cfg.dir.project, 'data', ['Groups' mod{i} '.mat']), 'file')
            def.data.(mod{i}).group.fname = fullfile(cfg.dir.project, 'data', ['Groups' mod{i} '.mat']);
        end
    end
end
if cfg.data.conf
...
for i=1:numel(cfg.data.mod)
    ...
    switch cfg.machine.name
        ...
        case 'grcca'
            % Hyperparameter name and type
            if isempty(cfg.machine.param.name)
                cfg.machine.param.name = {['L2' m] ['groupL2' m]};
            else
                if ~ismember(['L2' m], cfg.machine.param.name)
                    cfg.machine.param.name = [cfg.machine.param.name {['L2' m]}];
                end
                if ~ismember(['groupL2' m], cfg.machine.param.name)
                    cfg.machine.param.name = [cfg.machine.param.name {['groupL2' m]}];
                end
            end
            def.machine.param.type = 'factorial';

            % L2 regularization hyperparameters
            def.machine.param.(['rangeL2' m]) = [1 size(data.(upper(m)), upper(m), 2)^2];
            def.machine.param.(['rangegroupL2' m]) = [1 size(data.(upper(m)), upper(m), 2)^2];
    end
end
...
for i=1:numel(cfg.data.mod)
    ...
    switch cfg.machine.name
        ...
        case 'grcca'
            % Within-group and between-group L2 regularization hyperparameter for smoothing between CCA (l2=0) 
            % and PLS (l2=1) taking into account the group structure of the data
            % Note: 1 - logarithmic space for efficient L2 norm
            def.machine.param.(['L2' m]) = 1 - logspace(-log10(cfg.machine.param.(['rangeL2' m])(1)), ...
                -log10(cfg.machine.param.(['rangeL2' m])(2)), cfg.machine.param.(['nL2' m]));
            def.machine.param.(['groupL2' m]) = 1 - logspace(-log10(cfg.machine.param.(['rangegroupL2' m])(1)), ...
                -log10(cfg.machine.param.(['rangegroupL2' m])(2)), cfg.machine.param.(['ngroupL2' m]));
    end
end
...
for i=1:numel(cfg.data.mod)
    ...
    % Check if L2 regularization > 0
    if strcmp(cfg.machine.name, 'grcca') && ...
            ~all(ismember([cfg.machine.param.(['L2' m]) cfg.machine.param.(['groupL2' m])], 0))           
        if any(cfg.machine.param.(['L2' m]) <= 0)
            cfg.machine.param.(['L2' m])(cfg.machine.param.(['L2' m]) <= 0) = 1e-5;
            warning('Non-positive within-group L2 regularization is set to 1e-5')
        end
        if any(cfg.machine.param.(['groupL2' m]) <= 0)
            cfg.machine.param.(['groupL2' m])(cfg.machine.param.(['groupL2' m]) <= 0) = 1e-5;
            warning('Non-positive between-group L2 regularization is set to 1e-5')
        end
    end
end
